...
def main():
    ...
    logging.basicConfig(filename=f'./few-shot-d2a/d2a_Few_shot_samples_{samples}_{str(args.nums)}_seed_{seednum}log', level=logging.INFO,format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    for epoch in range(train_hps['num_epochs']):
        if epoch %1 == 0:
            # image_net_labels = generate_label_mapping_by_frequency(vision_model, reprogrammer, train_loader)
            # class_mapping = create_label_mapping(n_classes, args.m_per_class, image_net_labels)
            # print(class_mapping)
