...
import numpy as np
import pprint
import FLM_reprogramming_model
...
train_hps = {
    #default:100
    'num_epochs' : 100,
    'max_iterations' :100 , # overridden by args
...
def write_to_file(epoch,filename, type,acc, f1, prec, recall):
    with open(filename, 'a') as file:
        file.write(
            f'epoch{epoch}: {type} : acc = {100 * acc:.2f}% -------- F1 = {100 * f1:.2f}% -------- Precision = {100 * prec:.2f}% -------- Recall = {100 * recall:.2f}%\n')
...
def get_imagenet_label_list(vision_model, base_image, img_size):
    if base_image is None:
        torch.manual_seed(42)#default = 42
...
def main():
    ...
    print(f"args.nums == {args.nums}")
    logging.basicConfig(filename=f'./devign_flm_new/devign_{args.vision_model}_{str(args.nums)}_seed_{seednum}log', level=logging.INFO,format='%(asctime)s - %(levelname)s - %(message)s')
    filename = f'./devign_flm_new/devign_{args.vision_model}_seed_{seednum}log'
    ...
    from d2a_data_tokenize import Tokenize
    tokenizer = Tokenize()
    ...
    # train_dataset_raw = train_dataset_raw.shuffle(seed=42).select(range(50))
    # train_dataset_raw = datasets.load_dataset(path='csv', data_files='../dataset/reveal/reveal_train.csv', split='train')
    ...
    reprogrammer = FLM_reprogramming_model.ReprogrammingFuntion(vocab_size, args.img_patch_size,
        args.img_size,
    ...
    image_net_labels = generate_label_mapping_by_frequency(vision_model, reprogrammer, train_loader, args.m_per_class)
    # = create_label_mapping(n_classes, args.m_per_class, image_net_labels)
    class_mapping = create_label_mapping(n_classes, args.m_per_class, image_net_labels)
    # print()
    # print(class_mapping)
    for epoch in range(train_hps['num_epochs']):
        ...
        logging.debug(f'Epo {epoch} Training Lr: {scheduler.get_last_lr()[0]:.5f} Loss={loss_train:.4f}, Training Acc: {100 * acc:.2f}% F1Score:{100 * F1:.2f}% PrecisionScore: {100 * Precision:.2f}% RecallScore: {100 * Recall:.2f}% loss:{loss_train:.2f}')
        print(f"tp_num = {tp_num} tn_num = {tn_num} fp_num = {fp_num} fn_num = {fn_num}")
        write_to_file(epoch, filename, "Training:  ", acc, F1, Precision, Recall)
        #val
        all_labels = []
        all_prediction = []
        reprogrammer.eval()
        ...
        print(f"tp_num = {tp_num} tn_num = {tn_num} fp_num = {fp_num} fn_num = {fn_num}")
        logging.debug(f'Epo {epoch} Valing Lr: {scheduler.get_last_lr()[0]:.5f} Loss={loss_val:.4f}, Valing Acc: {100 * acc:.2f}% F1Score:{100 * F1:.2f}% PrecisionScore: {100 * Precision:.2f}% RecallScore: {100 * Recall:.2f}% loss:{loss_train:.2f}')
        write_to_file(epoch, filename, "Training:  ", acc, F1, Precision, Recall)
