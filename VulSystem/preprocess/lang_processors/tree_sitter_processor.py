...
#
from preprocess.lang_processors.lang_processor import LangProcessor
from preprocess.lang_processors.tokenization_utils import (
    process_string,
    replace_tokens,
    indent_lines,
)
...
class TreeSitterLangProcessor(LangProcessor):
    ...
    def create_treesiter_parser(self):
        if self.parser is None:
            lib_path = '/data/rqiu/multimodal_rerprogramming/VulSystem/preprocess/languages.so'
            # lib_path = 'D:\data_process\\VulSystem\\preprocess\\languages.so'
    ...
    def get_tokens_and_types(self, code):
        ...
        tokens = []
        tokens_type = []
        self.dfs(code, tree.root_node, tokens, tokens_type,depth=0)
    ...
    def dfs(self, code, node, tokens, tokens_type,depth):
        depth+=1
        if(depth >= 20):
            return
        ...
        for child in node.children:
            self.dfs(code, child, tokens, tokens_type,depth)
