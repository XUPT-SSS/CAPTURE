...
keywords = frozenset(['__asm', '__builtin', '__cdecl', '__declspec', '__except', '__export', '__far16', '__far32',
                      ...
                      'PQresultStatus','ifdef','endif','bool','void','ifndef','define'])
...
def clean_gadget(gadget):
    ...
    for line in gadget:
        ...
        if rx_comment.search(line) is None:
            # remove all string literals (keep the quotes) 2023.6.8 Comment out the following line
            nostrlit_line = re.sub(r'".*?"', '""', line)
            # remove all character literals                2023.6.8 Comment out the following line
